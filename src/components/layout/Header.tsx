...
export function Header({ title, subtitle }: HeaderProps) {
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6">
      {/* Left - Title */}
      <div>
        <h1 className="font-display font-semibold text-xl text-slate-900">
          {title}
        </h1>
        {subtitle && (
      ...
      <div className="flex-1 max-w-xl mx-8">
        <div className="relative">
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
            <kbd className="hidden sm:inline-flex px-1.5 py-0.5 text-[10px] font-medium text-slate-400 bg-slate-100 rounded">
              ⌘K
            </kbd>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {/* User Menu */}
        <button className="flex items-center gap-2 pl-3 pr-2 py-1.5 rounded-xl hover:bg-slate-100 transition-colors">
